...
CREATE OR REPLACE FUNCTION search.query_parser_fts(
        param_search_ou integer,
        param_depth integer,
        param_query text,
        param_statuses integer[],
        param_locations integer[],
        param_offset integer,
        param_check integer,
        param_limit integer,
        metarecord boolean,
        staff boolean,
        deleted_search boolean,
        param_pref_ou integer DEFAULT NULL::integer
) RETURNS SETOF search.search_result LANGUAGE plpgsql AS $function$
DECLARE
...
BEGIN
    ...
    IF param_search_ou > 0 THEN
        IF param_depth IS NOT NULL THEN
            SELECT array_accum(distinct id) INTO search_org_list FROM actor.org_unit_ancestors((actor.org_unit_ancestor_at_depth( param_search_ou, param_depth )).id);
        ELSE
            SELECT array_accum(distinct id) INTO search_org_list FROM actor.org_unit_ancestors(param_search_ou);
        END IF;
    ...
    LOOP
        ...
        IF NOT deleted_search THEN
            ...
            -- Fulfillment bases visiblity on bre.owner
            PERFORM 1
              FROM  biblio.record_entry
              WHERE id IN ( SELECT * FROM UNNEST( core_result.records ) )
                    AND ( owner NOT IN ( SELECT * FROM UNNEST( search_org_list ) ) OR owner IS NULL )
              LIMIT 1;

            IF NOT FOUND THEN
                -- RAISE NOTICE ' % were all owner-excluded ... ', core_result.records;
