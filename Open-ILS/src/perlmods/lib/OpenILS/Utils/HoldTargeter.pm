...
sub get_hold_copies {
    ...
    if ($hold_type eq 'C' || $hold_type eq 'R' || $hold_type eq 'F') {
        ...
        my $cp = $e->retrieve_asset_copy($hold_target);
        my $bibs = [$e->retrieve_asset_call_number($cp->call_number)->record];

    } elsif ($hold_type eq 'V') {

        $query->{where}->{'+acp'}->{call_number} = $hold_target;

        my $bibs = [$e->retrieve_asset_call_number($hold_target)->record];
        ...
        my $copies = [
            map {
                $_->target_copy
            } @{ $e->search_asset_copy_part_map({part=>$hold_target}) }
        ];
        my $bibs = [
            map {
                $e->retrieve_asset_call_number($_->call_number)->record
            } @{$e->search_asset_copy({id=>$copies})}
        ];
        ...
        my $copies = [
            map {
                $_->unit
            } @{ $e->search_serial_item({issuance=>$hold_target}) }
        ];
        my $bibs = [
            map {
                $e->retrieve_asset_call_number($_->call_number)->record
        ...
        my $bibs = [
            map {
                $_->source
            } @{ $e->search_metabib_metarecord_source_map({metarecord=>$hold_target}) }
        ];
    ...
    if (ref $bibs) {
        for my $bib_id (@$bibs) {
            my $bib = $e->retrieve_biblio_record_entry($bib_id);
            if (!$U->is_true($U->ou_ancestor_setting_value($bib->owner, 'ff.remote.connector.disabled'))) {
                FulfILLment::AT::Reactor::ItemLoad->ByBib( { target => $bib } );
            }
        }
    }
...
sub apply_copy_target {
    ...
    my $prev_copy = $hold->current_copy;
    ...
    # FF-----------------
    $prev_copy = $e->retrieve_asset_copy($prev_copy) if ($prev_copy);
    my $skip_target = 0;
    if ($prev_copy && $copy && ($copy->{id} == $prev_copy->id)) { # old copy and new copy are the same, leave it alone
        $skip_target = 1;
    } elsif ($prev_copy) { # old copy to remotely untarget, since we have a new one
        if (!$U->is_true($U->ou_ancestor_setting_value($prev_copy->circ_lib, 'ff.remote.connector.disabled')))    {
            $U->simplereq(
                "fulfillment.laicore",
                "fulfillment.laicore.hold.lender.delete_earliest",
                $prev_copy->source_lib,
                $prev_copy->barcode
            );
        }
    }

    if ($copy && !$skip_target) { # new copy to target
        if (!$U->is_true($U->ou_ancestor_setting_value($copy->{circ_lib}, 'ff.remote.connector.disabled')))    {
            my $new_copy = $e->retrieve_asset_copy($copy->{id});
            $U->simplereq(
                "fulfillment.laicore",
